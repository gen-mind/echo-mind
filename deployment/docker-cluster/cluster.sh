...
if [ -f "$SCRIPT_DIR/.env" ] && (grep -Eq "^[[:space:]]*ENABLE_LANGFUSE[[:space:]]*=[[:space:]]*true" "$SCRIPT_DIR/.env" 2>/dev/null || grep -q "ENABLE_LANGFUSE=true" "$SCRIPT_DIR/.env" 2>/dev/null); then
...
start_cluster() {
    ...
    docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE down 2>/dev/null || true
    docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE up -d
...
stop_cluster() {
    ...
    cd "$SCRIPT_DIR"
    docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE down
...
show_logs() {
    ...
    if [ -z "$1" ]; then
        ...
        docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE logs -f
    else
        log_info "Showing logs for service: $1 (Ctrl+C to exit)..."
        docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE logs -f "$1"
    fi
}
...
show_status() {
    ...
    # Get all containers
    ALL_CONTAINERS=$(docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE ps --format "{{.Name}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null)
...
pull_images() {
    ...
    cd "$SCRIPT_DIR"
    docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE pull
    ...
    if [ -z "$SERVICE" ]; then
        ...
        for svc in "${services[@]}"; do
            ...
            if [ -n "${HF_TOKEN:-}" ]; then
                ...
                docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE build --build-arg HF_TOKEN="$HF_TOKEN" "$svc"
            else
                docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE build "$svc"
            fi
        ...
        if [ -n "${HF_TOKEN:-}" ]; then
            ...
            docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE build --build-arg HF_TOKEN="$HF_TOKEN" "$SERVICE"
        else
            docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE build "$SERVICE"
        fi
...
rebuild_service() {
    ...
    if [ -n "${HF_TOKEN:-}" ]; then
        log_info "Using HF_TOKEN for build"
        docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE build --no-cache --build-arg HF_TOKEN="$HF_TOKEN" "$SERVICE"
    else
        docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE build --no-cache "$SERVICE"
    fi
    docker compose $COMPOSE_ENV_FLAG -f "$COMPOSE_FILE" $OBSERVABILITY_FILES $OBSERVABILITY_PROFILE $LANGFUSE_FILES $LANGFUSE_PROFILE $WEBUI_PROFILE up -d --force-recreate "$SERVICE"
